...
if [ ! -d "gfx" ]; then
    ...
    #sfk partcopy "$OUTPUT_ROM" -fromto 0x13Fb80 0x140180 gfx/menu_jap.bin -yes
    #sfk partcopy "$OUTPUT_ROM" -fromto 0x13Fae0 0x13Fc60 gfx/menu2_jap.bin -yes
    sfk partcopy "$OUTPUT_ROM" -fromto 0x13F780 0x13F820 gfx/menu_status_jap.bin -yes  # ステータス -> STATS
    sfk partcopy "$OUTPUT_ROM" -fromto 0x13F820 0x13F880 gfx/menu_shield_part_jap.bin -yes   # シルド (part of シールド) -> H[A]LD
    sfk partcopy "$OUTPUT_ROM" -fromto 0x13F880 0x13F8C0 gfx/menu_gold_part_jap.bin -yes  # ゴー (part of ゴールド) -> GO
    sfk partcopy "$OUTPUT_ROM" -fromto 0x13F8C0 0x13F920 gfx/menu_item_part_jap.bin -yes  # アイム (part of アイテム) -> ITEM
    sfk partcopy "$OUTPUT_ROM" -fromto 0x13F920 0x13F9a0 gfx/menu_magic_jap.bin -yes  # マジック -> MAGIC/SPELL
